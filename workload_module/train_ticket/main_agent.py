...
from poisson_rate_generator import poisson_rate_train_ticket
...
def run_program(rate, experiment_time, experiment_no, threads):
    ...
    for i in range(1, threads + 1):
        p = sp.Popen(
            [sys.executable, "train_ticket_api.py", rate, experiment_time,
...
def avoid_cold_start():
    path = 'log_data/cold_starts'
    if not os.path.exists(path):
        os.makedirs(path)
    run_program('60', '120', path, 1)
...
def program_runner():
    ...
    for x in rps_list:
        ...
        while True:
            ...
            rate = poisson_rate_train_ticket(RPS)
            # rate = poisson
