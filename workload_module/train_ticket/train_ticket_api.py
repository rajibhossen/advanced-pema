...
from static_data import USER_CREDETIALS, TRIP_DATA, TRAVEL_DATES
from train_ticket_admin_api import AdminActions
...
# token = "iRFwDhI9_Bje6tXMJG17Yn4_hydI7mNMmAPiEaBghc-3zpV6KmI7eQpPrAPi99GJJAOjIfC2tRVxE0ilL1oo2g=="
# org = "ridl"
# bucket = "response_latency"

# client = InfluxDBClient(url="http://192.168.2.11:8086", token=token)
...
class Requests:
    ...
    def __init__(self, logger, uuid):
        self.host = "http://192.168.2.62:32677"
        user = None  # random.choice(USER_CREDETIALS)
        self.user_name = user
        self.password = user
    ...
    def home(self):
        ...
        # point = Point("response_time").field("latency", response_time).time(arrival_time, WritePrecision.NS)
    ...
    def confirm_consign(self):
        ...
        response = requests.put(
            ...
            headers=head)
        response_time = time.time() - start_time
        to_log = str(arrival_time) + "," + "confirm_consign," + str(response.status_code) + "," + str(
            response_time) + "," + self.request_uuid
        self.logger.info(to_log)
